...
function renderQuestions(records, totalRecords, disabled = true) {
  ...
  byId("qa-list").innerHTML = records.length ? records.map((item, index) => {
    ...
    return `<article class="qa-card" data-id="${escapeHtml(item.id)}"><div class="question ${disabled ? "disabled" : ""}" data-number="${entryNumber}" role="button" tabindex="${disabled ? "-1" : "0"}" aria-expanded="false" aria-disabled="${disabled}"><div class="question-content markdown-content">${renderMarkdown(item.question)}</div><span class="question-toggle" aria-hidden="true">+</span></div><div class="answer"><div class="answer-content markdown-content"></div><p class="answer-attribution">This answer is generated by ${escapeHtml(item.answered_by)}.</p>${renderResources(item)}<div class="meta">${item.direct_tags.map((tag) => `<a class="tag" href="${tagFilterUrl(tag, item.collection)}" target="_blank" rel="noopener noreferrer">${escapeHtml(displayName(tag))} <span aria-hidden="true">↗</span></a>`).join("")}</div></div></article>`;
