...
function renderQuestions(records, totalRecords, disabled = true) {
  ...
  byId("qa-list").innerHTML = records.length ? records.map((item, index) => {
    ...
    return `<article class="qa-card" data-id="${escapeHtml(item.id)}"><div class="question ${item.is_frequent ? "frequent" : ""} ${disabled ? "disabled" : ""}" data-number="${entryNumber}" role="button" tabindex="${disabled ? "-1" : "0"}" aria-expanded="false" aria-disabled="${disabled}"><div class="question-content markdown-content">${renderMarkdown(item.question)}</div>${item.is_frequent ? `<span class="frequent-icon" role="img" aria-label="Frequent" title="Frequent">★</span>` : ""}<span class="question-toggle" aria-hidden="true">+</span></div><div class="answer"><div class="answer-content markdown-content"></div>${renderResources(item)}<div class="meta">${sortedDirectTags(item.direct_tags).map((tag) => `<a class="tag" href="${tagFilterUrl(tag, item.collection)}" target="_blank" rel="noopener noreferrer">${escapeHtml(displayName(tag))} <span aria-hidden="true">↗</span></a>`).join("")}</div><div class="entry-details"><span class="entry-author">Answered by ${escapeHtml(item.answered_by)}</span><span class="entry-id">${escapeHtml(item.id)}</span><span class="entry-created">${escapeHtml(formatUtcDate(item.created_at))} UTC</span></div></div></article>`;
